...
def add_cyclical_time_features():
  ...
  spark= SparkSession.builder.appName('silver3_features').master("local[*]").getOrCreate()
    # Pickup time
  try:
    ...
    output_path = "/opt/airflow/data/silver/cyclical_features"
    df.write.mode("overwrite").parquet(output_path)
    return "adjust cryclical time features done"
